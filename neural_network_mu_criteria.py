...
class LensDataset(Dataset): # torch.utils.data.Dataset
    ...
    def __getitem__(self, index):
        ...
        return image, criteria#n_sources.values[0]
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCH):
        ...
        for batch_idx, (data, criteria) in enumerate(tqdm(train_loader, total = len(train_loader))):
        ...
        with torch.no_grad():
            ...
            #     data, target = Variable(data).cuda(), Variable(target).cuda()
            #     data, target = data, target.unsqueeze(1)
            for batch_idx, (data, criteria) in enumerate(test_loader):
                data, target = data.float(), criteria.float()
                data, target = Variable(data).cuda(), Variable(target).cuda()
                data, target = data, target.unsqueeze(1)
            ...
            if total_loss/(total_counter) < best_accuracy:
                ...
                torch.save(net, save_model_path + datetime_today + 'criteria_' +'resnet18.mdl')
                print("saved to " + "criteria_resnet18.mdl" + " file.")
