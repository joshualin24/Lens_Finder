...
### loss function from JiWon Park (https://github.com/jiwoncpark/h0rton)
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCH):
        ...
        for batch_idx, (data, Einstein_Radius) in enumerate(tqdm(train_loader, total = len(train_loader))):
            ...
            output = net(data)
            #print("output:", output)
            loss = nll_diagonal(target=target, mu=output[:, 0].unsqueeze(1), logvar= output[:, 1].unsqueeze(1))#loss_fn(output, target)
            #m = nn.Sigmoid()
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, Einstein_Radius) in enumerate(test_loader):
                ...
                pred = net(data)
                loss = nll_diagonal(target=target, mu=pred[:, 0].unsqueeze(1), logvar= pred[:, 1].unsqueeze(1))#loss_fn(pred, target)
                #m = nn.Sigmoid()
            ...
            if total_loss/(total_counter) < best_accuracy:
                ...
                torch.save(net, save_model_path + datetime_today + 'nll_Einstein_Radius_' +'resnet18.mdl')
                print("saved to " + "nll_Einstein_Radius_resnet18.mdl" + " file.")
